...
class Level1():
    ...
    def setupWalls(self, wall_color):
        ...
        wall_positions = [
            ...
            [0, 600, 606, 6],#нижня
            [600, 0, 6, 273],#права1
            [600, 333, 6, 267],#права2
            [300, 0, 6, 66],#середня згори
            [60, 60, 186, 6],#1
            [360, 60, 186, 6],#2
            [60, 120, 66, 6],#3
            [60, 120, 6, 126],#6
    ...
    def setupRedFood(self, food_color,nocolor, bg_color):
        self.food_sprites = pygame.sprite.Group()
        for row in range(19):
            for col in range(19):
                if (row == 7 or row == 8) and (col == 8 or col == 9 or col == 10):
                    ...
                    food = Food(30 * col + 32, 30 * row + 32, 4, 4, nocolor, bg_color)
                    if randint(1,213) == 1 or randint(1,213) == 2 or randint(1,213) == 3:
                        food = Food(30 * col + 32, 30 * row + 32, 8, 8, food_color, bg_color)
                        cfg.food_coordinates.append([30 * col + 32,30 * row + 32])
                    is_collide = pygame.sprite.spritecollide(food, self.wall_sprites, False)
                    if is_collide:
                        continue
                    is_collide = pygame.sprite.spritecollide(food, self.hero_sprites, False)
